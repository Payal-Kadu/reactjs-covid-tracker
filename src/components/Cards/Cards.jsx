...
function Cards(props){
    const finalObjData = props.mydata;
    const {confirmed, recovered, deaths, lastUpdate} = finalObjData;
    
    //we wrote if because first time data id {} null so it throws error when we try to access values
    if(!confirmed)
    {
        return "Loading..."
    }
        ...
        <Grid container spacing={3} justify="center">
            <Grid item component={Card} xs={12} md={3} className={cx(styles.card, styles.infected)}>
                <CardContent>
                    ...
                    <Typography variant="h5" color="textSecondary">{new Date(lastUpdate).toDateString()}</Typography>
